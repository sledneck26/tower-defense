...
_quadtree.prototype.maxChilds = 3;
...
_quadtree.prototype.insert = function(item){
	if(this.spaces.length){
		var x1 = {x:item.currentLocation.x,y:item.currentLocation.y};
		var x2 = {x:item.currentLocation.x+item.w,y:item.currentLocation.y};
		var x3 = {x:item.currentLocation.x,y:item.currentLocation.y+item.h};
		var x4 = {x:item.currentLocation.x+item.w,y:item.currentLocation.y+item.h};
		for(i in this.spaces){
...
_quadtree.prototype.retrieve = function(item){
	//FIXME: should identify or index children to avoid duplicates
	if(this.spaces.length){
...
_quadtree.prototype.update = function(item){
